...
coefficients <- model_ridge %>%
  ...
  # put lambdas on log scale
  mutate(log_lambda = log(lambda)) %>%
  # add optimal lambda value
  mutate(lambda_star_ridge = log(lambda_star_ridge)) %>%
  # leave out incercept
...
# Save to PDF
ggsave(filename="PS06_MARCOS_BARROZO_ridge_coefficients.pdf", 
...
# Cross Validate!
cv_LASSO <- cv.glmnet(X, y, alpha = 1, lambda=lambda_values, nfolds = 10)
# Let's plot:
plot(cv_LASSO)
...
coefficients <- model_LASSO %>%
  ...
  mutate(log_lambda = log(lambda)) %>%
  # add optimal lambda value
  mutate(lambda_star_LASSO = log(lambda_star_LASSO)) %>%
  # leave out incercept
  filter(term != "(Intercept)")
...
# Save to PDF
ggsave(filename="PS06_FIRSTNAME_LASTNAME_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  # Fit models for all three methods
...
predictions <- model_LASSO %>%
  ...
  write_csv("PS06_MARCOS_BARROZO_submission.csv")