...
# Save to PDF
ggsave(filename="PS06_Rebeccca_Conover_ridge_coefficients.pdf", 
...
# Save to PDF
ggsave(filename="PS06_Rebecca_Conover_lasso_coefficients.pdf", 
...
results_all_folds <-  data.frame(method= c(), MSE=c(), fold=c())
...
for(i in 1:n_folds){
  ...
  y<-pseudo_test$SalePrice
  X_lm_new<-pseudo_test %>% 
    select(-SalePrice)
  
  lm_predictions <- model_lm %>% 
    predict(newdata=X_lm_new) 
...
sample_submission %>% 
  ...
  write_csv("PS06_Rebecca_Conover_submission.csv")

#doesnt let you submit with negative values, had to run until I got a prediction set with no negative values
