...
# making the model for ridge_regression
...
# Save to PDF
ggsave(filename="PS06_Brenda_Li_ridge_coefficients.pdf", 
...
# using cv to find optimal lambda
...
# Save to PDF
ggsave(filename="PS06_Brenda_Li_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  pseudo_train<-filter(train,fold!=i)
  pseudo_test<-filter(train,fold==i)
  
  X <- model.matrix(model_formula, data = pseudo_train)[, -1]
  y <- pseudo_train$SalePrice
  
  # making each model
  optimal_model_ridge <- glmnet(X, y, alpha = 0, lambda = lambda_star_ridge)
  optimal_model_LASSO <- glmnet(X, y, alpha = 1, lambda = lambda_star_LASSO)
  model_lm<-glm(model_formula,data=pseudo_train)
  ...
  ridge_predictions <- optimal_model_ridge %>% predict(newx=X_test)
  LASSO_predictions <- optimal_model_LASSO %>% predict(newx=X_test)
...
predictions <- predict(optimal_model_LASSO,newx = test_X)

# Finding the average of the predictions (this average is used to replace non-postive prediction values later)
positive_predictions<-as.data.frame(predictions) %>% filter(s0>0) 
average_prediction<-mean(positive_predictions$s0)

# Write submissions to CSV
sample_submission %>% 
  mutate(SalePrice = as.vector(predictions)) %>% 
  mutate(SalePrice = ifelse(SalePrice<=0,average_prediction,SalePrice)) %>% 
  write_csv("PS06_Brenda_Li_submission.csv")
