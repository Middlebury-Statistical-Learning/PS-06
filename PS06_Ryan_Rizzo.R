...
# and length values.
lambda_vals <- 10^seq(from = 2, to = 4, length = 2500)
...
model_ridge <- glmnet(X, y, alpha = 0, lambda = lambda_vals)
...
cv2 <- cv.glmnet(X, y, alpha = 0, lambda=lambda_vals, nfolds = 10)

cv2 %>%
  glance()
lambda_star_ridge <- cv2 %>%
  glance() %>%
  .[["lambda.min"]]
...
# Save to PDF
ggsave(filename="PS06_Ryan_Rizzo_ridge_coefficients.pdf", 
...
# Find the optimal lambda from these values. Feel free to toy with the from, to,
# and length values.
lambda_vals <- 10^seq(from = 1, to = 3.5, length = 2500)
...
#use cv to figure out the optimal lambda (bc we want the lowest MSE)

cv2 <- cv.glmnet(X, y, alpha = 1, lambda=lambda_vals, nfolds = 10)
...
# Save to PDF
ggsave(filename="PS06_Ryan_Rizzo_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  pseudoX <- model.matrix(model_formula, data = pseudo_train)[, -1]
  
  pseudoY <- pseudo_train$SalePrice
  
  newX <- model.matrix(model_formula, data = pseudo_test)[, -1]
  
  # Get predictions for all three methods.
  lm_predictions <- model_lm %>% 
    predict(newdata=pseudo_test)
  
  new_ridge <- glmnet(pseudoX, pseudoY, alpha = 0, lambda = lambda_star_ridge)
  
  ridge_predictions <- new_ridge %>%
    predict.glmnet(newx = newX, s = lambda_star_ridge)
  
  new_LASSO <- glmnet(pseudoX, pseudoY, alpha = 1, lambda = lambda_star_LASSO)
  
  LASSO_predictions <- new_LASSO %>%
    predict.glmnet(newx = newX, s = lambda_star_LASSO)
...
predictions <- predict(model_ridge, s = lambda_star_ridge, newx = test_X, type = "response")

# Write submissions to CSV
sample_submission %>% 
  mutate(SalePrice = as.vector(predictions)) %>% 
  write_csv("PS06_Ryan_Rizzo_submission.csv")