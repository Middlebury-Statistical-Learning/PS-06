...
# 1.2 Compute coefficients (Ridge) -----------------------------------------------
...
# Save to PDF
ggsave(filename="PS06_Nina_Sonneborn_ridge_coefficients.pdf", 
...
# 2.2 Compute Coefficients (LASSO) ---------------------------------------------
# Make LASSO model
model_LASSO <- glmnet(X, y, alpha = 1, lambda = lambda_values)

# The coefficients data frame
...
# Save to PDF
ggsave(filename="PS06_Nina_Sonneborn_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  # Create pseudo train and test datasets
  pseudo_train <- train %>% filter(fold != i)
  pseudo_test <- train %>% filter(fold == i)
  
  # Create models using optimal lambda values
  ...
  cv_model_ridge <- glmnet(X, y, alpha = 0, lambda = lambda_star_ridge)
  cv_model_LASSO <- glmnet(X, y, alpha = 1, lambda = lambda_star_LASSO)
  ...
  ridge_predictions <- cv_model_ridge %>% 
    predict(newx= X_new, s=lambda_star_ridge)
  LASSO_predictions <- cv_model_LASSO %>%
...
# I made this whole section into a function so that I can easily re-run it 
# and account for sampling variability
compare_methods()

# Outcome:
# Through all simulations, LASSO had smallest MSE, followed by ridge, then lm
...
# LASSO had the lowest cross-validated MSE:
optimal_model_LASSO <- glmnet(X, y, alpha = 1, lambda = lambda_star_LASSO)
predictions <-  optimal_model_LASSO %>%
  predict(newx=test_X, s=lambda_star_LASSO)

# Write submissions to CSV
sample_submission %>% 
  mutate(SalePrice = as.vector(predictions)) %>% 
  write_csv("PS06_Nina_Sonneborn_submission.csv")

# Note: Id 2872 had negative dollar amount prediction, so I manually changed it to zero
# so that kaggle would grade my submission

# Kaggle score: 0.43822
