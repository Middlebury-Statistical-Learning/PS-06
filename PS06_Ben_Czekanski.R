...
lambda_star_ridge <- cv.glmnet(X, y, alpha = 0, lambda = lambda_values, nfolds = n_folds)$lambda.min
...
# Save to PDF
ggsave(filename="PS06_Ben_Czekanski_ridge_coefficients.pdf", 
...
# Find the optimal lambda from these values. Feel free to toy with the from, to,
# and length values.
lambda_values <- 10^seq(from = 1, to = 15, length = 2500)
...
lambda_star_LASSO <- cv.glmnet(X, y, alpha = 1, lambda = lambda_values, nfolds = n_folds)$lambda.min
...
# Save to PDF
ggsave(filename="PS06_Ben_Czekanski_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  pseudo_X <- model.matrix(model_formula, data = pseudo_train)[, -1]
  pseudo_y <- pseudo_train$SalePrice
  ...
  model_ridge_new <- glmnet(pseudo_X, pseudo_y, alpha = 0, lambda = lambda_star_ridge)
  ridge_predictions <- model_ridge_new %>%
    predict.glmnet(newx = X_new, s = lambda_star_ridge)
  
  model_LASSO_new <- glmnet(pseudo_X, pseudo_y, alpha = 1, lambda = lambda_star_LASSO)
  LASSO_predictions <- model_LASSO_new %>%
    predict.glmnet(newx = X_new, s = lambda_star_LASSO)
...
predictions <- model_LASSO %>%
  predict.glmnet(newx = test_X, s = lambda_star_LASSO)
  
  # Write submissions to CSV
  sample_submission %>% 
  mutate(SalePrice = as.vector(predictions)) %>% 
  mutate(SalePrice = ifelse(SalePrice < 0, 0, SalePrice)) %>%
  write_csv("PS06_Ben_Czekanski_submission.csv")