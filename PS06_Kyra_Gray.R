...
setwd("~/Desktop/Middlebury/Stat Learning/PS06")
...
# Save to PDF
ggsave(filename="PS06_Kyra_Gray_ridge_coefficients.pdf", 
...
lambda_values <- 10^seq(from = 1, to = 3.5, length = 100)
...
# Save to PDF
ggsave(filename="PS06_Kyra_Gray_LASSO_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  specific_lambdas_ridge <- c(lambda_star_ridge)
  specific_lambdas_LASSO <- c(lambda_star_LASSO)
  
  # Get predictions for all three methods.
  lm_predictions <- model_lm %>% 
    predict(newdata=pseudo_test)
  ridge_predictions <- model_ridge %>% 
    predict(newx=pseudo_test_X, s=specific_lambdas_ridge)
  LASSO_predictions <-model_LASSO %>% 
    predict(newx=pseudo_test_X, s=specific_lambdas_LASSO)
...
sample_submission %>% 
  ...
  write_csv("PS06_Kyra_Gray_submission.csv")