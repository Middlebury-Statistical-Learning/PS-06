...
setwd("~/Desktop/Junior Spring/Statistical Learning/Homework/HW6")
...
# Save to PDF
ggsave(filename="PS06_Aayam_Poudel_ridge_coefficients.pdf", 
...
# Save to PDF
ggsave(filename="PS06_Aayam_Poudel_lasso_coefficients.pdf", 
...
train_X <- model.matrix(model_formula, data = train)[, -1]
...
model_LASSO <- glmnet(train_X, train_y, alpha = 1, lambda = lambda_star_LASSO)
...
sample_submission %>% 
  ...
  write_csv("PS06_Aayam_Poudel_submission.csv")