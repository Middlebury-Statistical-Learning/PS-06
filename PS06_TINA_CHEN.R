...
#  select(estimate) %>% 
#  mutate(lambda_star_ridge = min(estimate))
...
ggplotly()
# Save to PDF
ggsave(filename="PS06_TINA_CHEN_ridge_coefficients.pdf", 
...
ggplotly()
# Save to PDF
ggsave(filename="PS06_TINA_CHEN_lasso_coefficients.pdf", 
...
for(i in 1:n_folds){
  ...
  model_lm <- lm(model_lm, data = train)
  model_ridge <- glmnet(X, y, alpha = 0, lambda = lambda_values)
  model_LASSO <- glmnet(X, y, alpha = 1, lambda = lambda_values)
  ...
  LASSO_predictions <- predict(model_ridge, s = lambda_star_LASSO, newx = X_test, type = "response")
...
sample_submission %>% 
  ...
  write_csv("PS06_TINA_CHEN_submission.csv")