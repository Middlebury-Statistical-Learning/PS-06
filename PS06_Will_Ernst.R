...
# Save to PDF
ggsave(filename="PS06_Will_Ernst_ridge_coefficients.pdf", 
...
# Save to PDF
ggsave(filename="PS06_Will_Ernst_lasso_coefficients.pdf", 
...
predictions <- model_LASSO %>% 
  predict(newx=test_X, s=specific_lambdas_LASSO)

# Write submissions to CSV
sample_submission %>% 
  mutate(SalePrice = as.vector(predictions)) %>% 
  write_csv("PS06_Will_Ernst_submission.csv")
